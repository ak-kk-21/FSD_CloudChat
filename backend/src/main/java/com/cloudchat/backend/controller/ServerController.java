...
public class ServerController {
    ...
    @GetMapping("/public")
    public ResponseEntity<?> getPublicServers() {
        List<Server> servers = serverRepository.findByIsPublicTrue();
    ...
    public ResponseEntity<?> createServer(@RequestBody Server server, @RequestHeader("userId") String userId) {
        ...
        // Generate automatic invite code
    ...
    public ResponseEntity<?> joinViaInvite(
            @PathVariable("code") String code, 
            @RequestHeader("userId") String userId) {
        
        Optional<Server> serverOpt = serverRepository.findByInviteCode(code);
        
        if (serverOpt.isEmpty()) {
            Map<String, String> error = new HashMap<>();
            error.put("error", "Invalid or expired invite code");
            return ResponseEntity.status(404).body(error);
        }
        ...
        if (server.getMembers().contains(userId)) {
            Map<String, String> error = new HashMap<>();
            error.put("error", "Already a member of this server");
            return ResponseEntity.badRequest().body(error);
        }
    ...
    public ResponseEntity<?> leaveServer(
        ...
        server.getMembers().remove(userId);
        
        // If it was the owner, we could delete it, but for now we just remove them
        // or transfer ownership. Let's just remove them for simplicity.
        
        serverRepository.save(server);
        
        Map<String, String> response = new HashMap<>();
        response.put("message", "Left server successfully");
